...
If you’ve ever thought “the model is wrong”, this evaluator helps you answer the real question:
**Was the answer wrong… or did we feed it broken text?**
...
### How to use these metrics in a RAG pipeline

Practical ways teams use these numbers:

- **Gate ingestion**: if a document’s WER/CER is high (or many pages are flagged low-quality), push it to an OCR queue instead of indexing it immediately.
- **Tune chunking**: poor line-F1 often correlates with broken headings and hard line-wraps; fixing that before chunking tends to improve retrieval more than changing embedding models.
- **Cost-control LLM cleanup**: run cleanup only on pages flagged low-quality, rather than on every page.
- **Regression testing**: if you change parsing settings (or update dependencies), rerun the evaluator and make sure metrics don’t silently degrade.

Important note: when ground truth is generated by an LLM, the evaluation is still useful for *relative* comparisons (A vs B), but it’s not the same as human-labeled gold data. For high-stakes documents, spot-check a few pages manually.
